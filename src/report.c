...
Stats *s;
{
  ...
  Sectionstats *ssp;
  int i, n;
  float r, min, max;  //q not being used so removing 
  ...
  fprintf(fd, "QUANTILE SUMMARIES\n\n");
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           ...
           min = scores[0];
           max = scores[n-1];
           for(long unsigned int i = 0; i < sizeof(quantiles)/sizeof(*quantiles); i++) {
             fprintf(fd, "   ");
...
void
reporttabs(FILE *fd, Course *c)  //removing int nm from the function
{
        Assignment *ap;
        Student *stp;
        Score *rscp;  //removing *nscp
