...
static char *short_options = "rcan:k";
...
char *argv[];
{
        ...
        Stats *s;
        char optval;
        int (*compare)(Student *, Student *) = comparename;
        ...
        while(optind < argc) {
            if((optval = getopt_long(argc, argv, short_options, long_options, NULL)) != -1) {
                ...
                case OUTPUT:
                    output++;
                    outputfile =freopen(optarg,"w",stdout);
                    break;
                case 'o':
                    output++;
                    outputfile =freopen(optarg,"w",stdout);
        ...
        if(histograms) reporthistos(stdout, c, s,nonames);
        if(scores) reportscores(stdout, c, nonames);
        if(tabsep) reporttabs(stdout, c); //removing nonames
